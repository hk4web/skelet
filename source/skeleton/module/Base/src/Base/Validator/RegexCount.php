<?php

namespace Docapost\Base\Validator;

use Zend\Stdlib\ErrorHandler;
use Zend\Stdlib\StringUtils;
use Zend\Stdlib\StringWrapper\StringWrapperInterface as StringWrapper;
use Zend\Validator\AbstractValidator;

class RegexCount extends AbstractValidator
{
    const INVALID   = 'regexCountInvalid';
    const TOO_SHORT = 'regexCountTooShort';
    const TOO_LONG  = 'regexCountTooLong';
    const ERROROUS  = 'regexCountErrorous';

    /**
     * @var array
     */
    protected $messageTemplates = [
        self::INVALID   => "Invalid type given. String, integer or float expected",
        self::TOO_SHORT => "The input match less than %min% times against pattern '%pattern%'",
        self::TOO_LONG  => "The input match more than %min% times against pattern '%pattern%'",
        self::ERROROUS  => "There was an internal error while using the pattern '%pattern%'",
    ];

    /**
     * @var array
     */
    protected $messageVariables = [
        'pattern' => ['options' => 'pattern'],
        'min'     => ['options' => 'min'],
        'max'     => ['options' => 'max'],
    ];

    protected $options = [
        'pattern'  => '/[.]/',   // Regular expression to count
        'min'      => 0,       // Minimum length
        'max'      => null,    // Maximum length, null if there is no length limitation
        'encoding' => 'UTF-8', // Encoding to use
    ];

    protected $stringWrapper;

    /**
     * Sets validator options
     *
     * @param  int|array|\Traversable $options
     */
    public function __construct($options = [])
    {
        if (!is_array($options)) {
            $options         = func_get_args();
            $temp['pattern'] = array_shift($options);
            if (!empty($options)) {
                $temp['min'] = array_shift($options);
            }
            if (!empty($options)) {
                $temp['max'] = array_shift($options);
            }
            if (!empty($options)) {
                $temp['encoding'] = array_shift($options);
            }

            $options = $temp;
        }

        parent::__construct($options);
    }

    /**
     * Returns the actual pattern
     *
     * @return string
     */
    public function getPattern()
    {
        return $this->options['pattern'];
    }

    /**
     * Sets the pattern
     *
     * @param string $pattern
     * @return RegexCount
     * @throws Exception\InvalidArgumentException
     */
    public function setPattern($pattern)
    {
        ErrorHandler::start();
        $this->options['pattern'] = (string) $pattern;
        $status = preg_match($this->options['pattern'], "Test");
        $error  = ErrorHandler::stop();

        if (false === $status) {
            throw new \Docapost\Base\Exception\InvalidArgumentException(
                "Internal error parsing the pattern '{$this->options['pattern']}'",
                0,
                $error
            );
        }

        return $this;
    }

    /**
     * Returns the min option
     *
     * @return int
     */
    public function getMin()
    {
        return $this->options['min'];
    }

    /**
     * Sets the min option
     *
     * @param  int $min
     * @throws Exception\InvalidArgumentException
     * @return RegexCount Provides a fluent interface
     */
    public function setMin($min)
    {
        if (null !== $this->getMax() && $min > $this->getMax()) {
            throw new Exception\InvalidArgumentException(
                "The minimum must be less than or equal to the maximum length, but {$min} > {$this->getMax()}"
            );
        }

        $this->options['min'] = max(0, (int) $min);
        return $this;
    }

    /**
     * Returns the max option
     *
     * @return int|null
     */
    public function getMax()
    {
        return $this->options['max'];
    }

    /**
     * Sets the max option
     *
     * @param  int|null $max
     * @throws Exception\InvalidArgumentException
     * @return RegexCount Provides a fluent interface
     */
    public function setMax($max)
    {
        if (null === $max) {
            $this->options['max'] = null;
        } elseif ($max < $this->getMin()) {
            throw new Exception\InvalidArgumentException(
                "The maximum must be greater than or equal to the minimum length, but {$max} < {$this->getMin()}"
            );
        } else {
            $this->options['max'] = (int) $max;
        }

        return $this;
    }

    /**
     * Get the string wrapper to detect the string length
     *
     * @return StringWrapper
     */
    public function getStringWrapper()
    {
        if (!$this->stringWrapper) {
            $this->stringWrapper = StringUtils::getWrapper($this->getEncoding());
        }
        return $this->stringWrapper;
    }

    /**
     * Set the string wrapper to detect the string length
     *
     * @param StringWrapper $stringWrapper
     * @return RegexCount
     */
    public function setStringWrapper(StringWrapper $stringWrapper)
    {
        $stringWrapper->setEncoding($this->getEncoding());
        $this->stringWrapper = $stringWrapper;
    }

    /**
     * Returns the actual encoding
     *
     * @return string
     */
    public function getEncoding()
    {
        return $this->options['encoding'];
    }

    /**
     * Sets a new encoding to use
     *
     * @param string $encoding
     * @return RegexCount
     * @throws Exception\InvalidArgumentException
     */
    public function setEncoding($encoding)
    {
        $this->stringWrapper = StringUtils::getWrapper($encoding);
        $this->options['encoding'] = $encoding;
        return $this;
    }

    /**
     * Returns true if and only if the string length of $value is at least the min option and
     * no greater than the max option (when the max option is not null).
     *
     * @param  string $value
     * @return bool
     */
    public function isValid($value)
    {
        if (!is_string($value) && !is_int($value) && !is_float($value)) {
            $this->error(self::INVALID);
            return false;
        }

        $this->setValue($value);

        ErrorHandler::start();
        $count = preg_match_all($this->pattern, $value);
        ErrorHandler::stop();

        if (false === $count) {
            $this->error(self::ERROROUS);
            return false;
        }

        if ($count < $this->getMin()) {
            $this->error(self::TOO_SHORT);
        }

        if (null !== $this->getMax() && $this->getMax() < $count) {
            $this->error(self::TOO_LONG);
        }

        if (count($this->getMessages())) {
            return false;
        }

        return true;
    }
}